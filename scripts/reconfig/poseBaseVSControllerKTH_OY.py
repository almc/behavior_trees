...
class poseBaseVSController:
    def __init__(self, name):
        ...
        # Create publishers
        self.pub_vel = rospy.Publisher('/oy/cmd_vel', Twist)
        self.pub_confirm = rospy.Publisher('activity_done_OY', confirmation)
        
        # Create Subscriber
        #rosOY.Subscriber("/cur_pose_C", pose, self.updatePose)
        rospy.Subscriber("/ceiling/pose", ARMarkersExtended, self.updateRobotsPose)
        rospy.Subscriber("/next_move_OY", activity, self.updateNextMove)
...
if __name__ == '__main__':
    try:
        rospy.init_node('poseBaseVSController_OY')
